...
+ (NSString *)uniqueDeviceToken
{
    @try
    {
        KeychainItemWrapper *keychainItem = [[KeychainItemWrapper alloc] initWithIdentifier:(__bridge id)kSecValueData accessGroup:nil];
        if([[keychainItem objectForKey:(__bridge id)kSecValueData] length]==0)
        {
            NSString* strUUID = DeviceUUID;
            [keychainItem setObject:strUUID forKey:(__bridge id)kSecValueData];
        }
        
        NSString *str=@"";
        str=[keychainItem objectForKey:(__bridge id)kSecValueData];
        keychainItem=nil;
        return str;
    }
